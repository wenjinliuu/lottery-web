...
- 开奖数据：只读取 `lottery-data-repo/public_data/v2` 的 GitHub V2 镜像，不直接连接 CloudBase，也不使用 V1 文件
...
- 自动开奖：CloudBase 负责抓取与入库，独立数据仓库 `lottery-data-repo` 在 08:14 兜底任务中导出 GitHub V2 镜像
- 下期开奖：日历 API 延迟时显示仓库推算值，官方数据更新后自动确认或纠正待开奖记录
- 数据状态：“设置”页面保留简洁入口，点开查看 GitHub V2 镜像、日历、下期状态、PWA 和备份状态
...
- 不需要配置 `JISU_APPKEY`
- 前端会直接读取 `lottery-data-repo/public_data/v2` 的公开开奖 JSON
