...
assert.match(html, /styles\.css\?v=3\.5\.5/);
...
assert.match(app, /entry-mode-icon/, "the four manual-entry modes should keep visual icons");
...
assert.match(styles, /@keyframes wallet-firework-soft/, "winning tickets should use a soft firework celebration");
...
assert.match(styles, /\.wallet-ticket \.wallet-draw-panel[\s\S]*border:\s*1px solid/, "draw number panels should use a soft solid ticket border");
...
assert.match(styles, /\.wallet-ticket\.is-winning[\s\S]*--ticket-outline-color:\s*rgba\(16,185,129,.48\)/, "winning tickets should keep a visible restrained green outline");
assert.match(styles, /--ticket-outline-color:\s*rgba\(var\(--wallet-rgb\),\s*\.42\)/, "ticket outline should follow the lottery color");
assert.match(styles, /\.wallet-ticket::before[\s\S]*stroke-width='1\.35'/, "ticket outline should trace the left irregular edge");
assert.match(styles, /\.wallet-ticket::after[\s\S]*stroke-width='1\.35'/, "ticket outline should trace the right irregular edge");
...
assert.match(styles, /-webkit-mask-size:\s*calc\(100% - 20px\)/, "the center mask must leave both subtle torn edges visible");
assert.match(styles, /\.wallet-ticket[\s\S]*border-radius:\s*0 !important/, "wallet ticket must not retain a rounded-card contour");
assert.match(styles, /\.wallet-ticket::before,[\s\S]*content:\s*"" !important/, "masked side outlines should be enabled without legacy notches");
assert.match(styles, /\.wallet-ticket-delete[\s\S]*border-radius:\s*50%/, "ticket delete control should be a compact icon circle");
assert.match(app, /class="delete-btn wallet-ticket-delete"[^>]*>\$\{ICON\.trash\}<\/button>/, "ticket delete control must render only the trash icon");
